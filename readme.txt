...
HOW TO USE IT: If you want only to compile the C files, you can type in the compiler "nmake find".
If you want to see how the tests are working, you can type in nmake and the number of test. 
For example, in order to run test5 from the makefile, you have to write in the compiler "nmake test5".
If you want to type in all arguments in the compiler, first you have to make sure that you have compiled the C files.
Then you can type in what you want, following the instructions below.
...
If you want to store the results from the search in an "output.txt" file, you must use the '-o' argument. For example,
to store the output from the search in a "output.txt" file, you have to write "find time -i book.txt -o output.txt".
...
For example, you can type in commands such as "tasklist" or "dir" and they will provide the text to search in. If you want to use this Command Line Tool
...
"tasklist | find chrome -o output.txt". You can find another example in the makefile as "test10". 
...
If you are looking for the word "brother" in a "book.txt" file and you want to ignore the case, you must write "find brother -i book.txt -c".
...
ii) What is more, examples "test6","test7" and "test8" show that, if your input file does not exist or is invalid or the output file is invalid
the Command Line Tool will stop and an error message will occure.
iii) From example "test11" you can see that if you type in CMD command and in the same time enter an input file, the Command Line Tool will read from the input file.